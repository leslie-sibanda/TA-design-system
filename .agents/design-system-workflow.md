# Design System Agent Workflow

Use this workflow for every component, pattern, theme, or documentation change.

1. Read the active delivery brief and contract.
2. Identify whether the request affects a foundation, generic component, reusable pattern, app theme, or documentation only.
3. Check existing components and registry items before introducing a new API.
4. Implement in the canonical package directory.
5. Exercise all states in the theme lab and supported app themes.
6. Update public docs, examples, registry metadata, and MCP resources together.
7. Run deterministic checks before agent evaluations.
8. Record findings, decisions, and remaining work in the active delivery review and handoff.

Escalate to an ADR when a change alters the primitive library, token contract, component ownership, distribution model, agent security boundary, or compatibility policy.

